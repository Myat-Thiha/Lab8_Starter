...
function initializeServiceWorker() {
  ...
  if ('serviceWorker' in navigator) {
    // If it is supported, we can register our service worker
    console.log('Service Worker is supported');
  } else {
    console.warn('Service Worker is not supported in this browser');
    return; // Exit if service workers are not supported
  }
  ...
  //            "Using Service Workers" will help you here)
    navigator.serviceWorker.register('./sw.js')
      .then((registration) => {
        // B4. TODO - Once the service worker has been successfully registered,
        //            console log that it was successful.
        console.log('Service Worker registered successfully with scope:', registration.scope);
      })
      .catch((error) => {
        // B5. TODO - In the event that the service worker registration fails,
